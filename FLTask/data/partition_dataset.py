...
import os
np.random.seed(42)
from .cifar10 import get_all_targets_cifar10, get_dataloader_cifar10
...
def data_distributer(
    ...
    elif partition == "sharding":
        net_dataidx_map, rand_set_all = sharding_partition(
            all_targets, n_clients, shards
        )
    ...
    elif partition == "sharding_max":
        net_dataidx_map = sharding_max_partition(all_targets, n_clients, shards)
    elif partition == "niid":
        print("niid")
        net_dataidx_map = lda_partition(all_targets, n_clients, dir)
        #net_dataidx_map = split_noniid(all_targets, n_clients, partition.alpha)
    else:
    ...
    global_loaders = {
        "train": DATA_LOADERS[dataset_name](root, train=False, batch_size=batch_size),
    ...
    for client_idx in range(n_clients):
        ...
        while len(idx_by_devices) < K:
            ...
            if (
                ...
                del tot_idx_by_label[chosen_label][l_idx]  # ?? shard? ??? ??!
...
def split_noniid(all_targets, n_clients, alpha):
    n_classes = all_targets.max() + 1
    label_distribution = np.random.dirichlet([alpha] * n_clients, n_classes)

    class_idcs = [np.argwhere(all_targets == y).flatten()
