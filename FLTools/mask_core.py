...
#     parser.add_argument('--sparse_init', type=str, default='snip', help='sparse initialization') #uniform/uniform_plus/ERK/ERK_plus/ER/snip/GraSP
...
#     parser.add_argument('--death-rate', type=float, default=0.5, help='The pruning rate / death rate used for dynamic sparse training (not used in this paper).')
...
class Masking(object):
    ...
    def init(self, mode='ERK', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'global_magnitude':
            ...
            total_sparse_params = total_sparse_params - self.masks['conv2d_1.weight'].numel()
            self.masks.pop('conv2d_1.weight')

            if self.density < 0.2:
                total_sparse_params = total_sparse_params - self.masks['classifier.weight'].numel() * 0.2
